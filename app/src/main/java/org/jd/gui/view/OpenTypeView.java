...
public class OpenTypeView {
    ...
    protected JLabel openTypeEmptyStateLabel;
    protected JList<OpenTypeListCellBean> openTypeList;
    ...
    public OpenTypeView(API api, JFrame mainFrame, Consumer<String> changedPatternCallback, TriConsumer<Point, Collection<Container.Entry>, String> selectedTypeCallback) {
        ...
        SwingUtil.invokeLater(() -> {
            ...
            // Empty-state label — shown when the list model has no items
            openTypeEmptyStateLabel = new JLabel("Start typing a class name…", SwingConstants.CENTER);
            openTypeEmptyStateLabel.setForeground(UIManager.getColor("Label.disabledForeground"));
            openTypeEmptyStateLabel.setFont(openTypeEmptyStateLabel.getFont().deriveFont(Font.ITALIC));

            JPanel centerPanel = new JPanel(new BorderLayout());
            centerPanel.add(scrollPane, BorderLayout.CENTER);
            centerPanel.add(openTypeEmptyStateLabel, BorderLayout.SOUTH);
    ...
    public void updateList(Map<String, Collection<Container.Entry>> map) {
        SwingUtil.invokeLater(() -> {
            DefaultListModel<OpenTypeListCellBean> model = (DefaultListModel<OpenTypeListCellBean>)openTypeList.getModel();
            ...
            for (String typeName : typeNames) {
                if (index < MAX_LINE_COUNT) {
                    ...
                    // Show a readable overflow hint instead of a null sentinel
                    int overflow = typeNames.size() - MAX_LINE_COUNT;
                    String msg = "\u2026 and " + overflow + " more result" + (overflow > 1 ? "s" : "") + ". Refine your search.";
                    model.addElement(new OpenTypeListCellBean(msg, null, null, null, null));
                }
                index++;
            }
            ...
            openTypeEmptyStateLabel.setVisible(count == 0 && !map.isEmpty());
