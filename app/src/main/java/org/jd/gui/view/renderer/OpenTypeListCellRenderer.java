    ...
    public Component getListCellRendererComponent(JList<? extends OpenTypeListCellBean> list, OpenTypeListCellBean value, int index, boolean selected, boolean hasFocus) {
        if (value != null) {
            if (value.entries == null) {
                // Overflow / "…and N more results" hint entry — render in italic grey, non-selectable
                label.setText(value.label);
                label.setIcon(null);
                info.setText("");

                label.setFont(label.getFont().deriveFont(Font.ITALIC));
                label.setForeground(infoNonSelectionColor);
            ...
            // Safety fallback for any unexpected null value
            label.setText("");
            label.setIcon(null);
            info.setText("");
            label.setForeground(textNonSelectionColor);
