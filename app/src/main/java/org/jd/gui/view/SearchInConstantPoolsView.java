...
public class SearchInConstantPoolsView<T extends DefaultMutableTreeNode & ContainerEntryGettable & UriGettable> {
    ...
    public static final int SEARCH_CASE_INSENSITIVE = 256;
    public static final int SEARCH_REGEX = 512;
    public static final int SEARCH_EXACT_MATCH = 1024;
    public static final int SEARCH_RESOURCES = 2048;

    protected API api;
    protected Set<URI> accepted = new HashSet<>();
    protected Set<T> expanded = new HashSet<>();
    protected boolean isPopulatingHistory = false;

    protected JDialog searchInConstantPoolsDialog;
    protected JComboBox<String> searchInConstantPoolsEnterComboBox;
    protected JTextField searchInConstantPoolsEnterTextField;
    protected JComboBox<String> searchInConstantPoolsFileMaskComboBox;
    ...
    protected JCheckBox searchInConstantPoolsCheckBoxResources;
    protected JCheckBox searchInConstantPoolsCheckBoxDeclarations;
    protected JCheckBox searchInConstantPoolsCheckBoxReferences;
    protected JCheckBox searchInConstantPoolsCheckBoxCaseInsensitive;
    protected JCheckBox searchInConstantPoolsCheckBoxRegex;
    protected JCheckBox searchInConstantPoolsCheckBoxExactMatch;
    ...
    public SearchInConstantPoolsView(
        ...
        SwingUtil.invokeLater(() -> {
            ...
            // Text field (upgraded to editable combobox for history)
            searchInConstantPoolsEnterComboBox = new JComboBox<>();
            searchInConstantPoolsEnterComboBox.setEditable(true);
            vbox.add(searchInConstantPoolsEnterComboBox);
            searchInConstantPoolsEnterTextField = (JTextField)searchInConstantPoolsEnterComboBox.getEditor().getEditorComponent();
            ...
            hbox.add(new JLabel("File mask (e.g. *.xml, *.properties):"));
            hbox.add(Box.createHorizontalGlue());
            vbox.add(hbox);

            vbox.add(Box.createVerticalStrut(5));
            String[] fileMaskPresets = {"*", "*.class", "*.xml", "*.properties", "*.json", "*.yml;*.yaml", "*.txt"};
            searchInConstantPoolsFileMaskComboBox = new JComboBox<>(fileMaskPresets);
            searchInConstantPoolsFileMaskComboBox.setEditable(true);
            vbox.add(searchInConstantPoolsFileMaskComboBox);

            searchInConstantPoolsFileMaskComboBox.addActionListener(e -> {
                if (!isPopulatingHistory) {
                    changedPatternCallback.accept(searchInConstantPoolsEnterTextField.getText(), getFlags());
                }
            });
            JTextField maskEditor = (JTextField)searchInConstantPoolsFileMaskComboBox.getEditor().getEditorComponent();
            maskEditor.getDocument().addDocumentListener(new DocumentListener() {
                @Override public void insertUpdate(DocumentEvent e) { call(); }
                @Override public void removeUpdate(DocumentEvent e) { call(); }
                @Override public void changedUpdate(DocumentEvent e) { call(); }
                protected void call() {
            ...
            subsubpanel = new JPanel();
            subsubpanel.setLayout(new GridLayout(2, 1));
            subsubpanel.add(searchInConstantPoolsCheckBoxResources = new JCheckBox("Resources", true));
            searchInConstantPoolsCheckBoxResources.addItemListener(checkBoxListener);
            subhbox.add(subsubpanel);
            ...
            subpanel = new JPanel();
            subpanel.setBorder(BorderFactory.createTitledBorder("Options"));
            subpanel.setLayout(new BorderLayout());
            hbox.add(subpanel);
            ...
            subsubpanel.add(searchInConstantPoolsCheckBoxCaseInsensitive = new JCheckBox("Case Insensitive", true));
            searchInConstantPoolsCheckBoxCaseInsensitive.addItemListener(checkBoxListener);
            subsubpanel.add(searchInConstantPoolsCheckBoxRegex = new JCheckBox("Regex"));
            searchInConstantPoolsCheckBoxRegex.addItemListener(e -> {
                // Regex mode and case-insensitive interact: refresh search
                changedPatternCallback.accept(searchInConstantPoolsEnterTextField.getText(), getFlags());
                searchInConstantPoolsEnterTextField.requestFocus();
            });
            subsubpanel.add(searchInConstantPoolsCheckBoxExactMatch = new JCheckBox("Exact Match"));
            searchInConstantPoolsCheckBoxExactMatch.addItemListener(checkBoxListener);
            subhbox.add(subsubpanel);
    ...
    protected void populate(DefaultTreeModel model, T node) {
        ...
        while (i-- > 0) {
            ...
            if ((child instanceof TreeNodeExpandable) && !expanded.contains(child)) {
                populate(child);
            }
        }
        // Refresh
    ...
    protected void populate(T node) {
        if ((node instanceof TreeNodeExpandable) && !expanded.contains(node)) {
            // Populate
    ...
    public int getFlags() {
        ...
        if (searchInConstantPoolsCheckBoxCaseInsensitive.isSelected())
            flags += SEARCH_CASE_INSENSITIVE;
        if (searchInConstantPoolsCheckBoxRegex.isSelected())
            flags += SEARCH_REGEX;
        if (searchInConstantPoolsCheckBoxExactMatch != null && searchInConstantPoolsCheckBoxExactMatch.isSelected())
            flags += SEARCH_EXACT_MATCH;
        if (searchInConstantPoolsCheckBoxResources != null && searchInConstantPoolsCheckBoxResources.isSelected())
            flags += SEARCH_RESOURCES;

        return flags;
    }

    public String getFileMask() {
        if (searchInConstantPoolsFileMaskComboBox == null) return "*";
        Object selected = searchInConstantPoolsFileMaskComboBox.getSelectedItem();
        return selected != null ? selected.toString().trim() : "*";
    }

    public void populateSearchHistory(Collection<String> history, String currentPattern) {
        SwingUtil.invokeLater(() -> {
            if (searchInConstantPoolsEnterComboBox == null) return;
            
            isPopulatingHistory = true;
            try {
                // Temporarily remove action listeners to prevent triggering search updates
                ActionListener[] listeners = searchInConstantPoolsEnterComboBox.getActionListeners();
                for (ActionListener l : listeners) {
                    searchInConstantPoolsEnterComboBox.removeActionListener(l);
                }

                searchInConstantPoolsEnterComboBox.removeAllItems();
                if (history != null) {
                    for (String item : history) {
                        searchInConstantPoolsEnterComboBox.addItem(item);
                    }
                }

                searchInConstantPoolsEnterComboBox.setSelectedItem(currentPattern != null ? currentPattern : "");

                // Restore action listeners
                for (ActionListener l : listeners) {
                    searchInConstantPoolsEnterComboBox.addActionListener(l);
                }
            } finally {
                isPopulatingHistory = false;
            }
        });
    }

    public void populateFileMaskHistory(Collection<String> history, String currentMask) {
        SwingUtil.invokeLater(() -> {
            if (searchInConstantPoolsFileMaskComboBox == null) return;
            ...
            try {
                // Temporarily remove action listeners to prevent triggering search updates
                ActionListener[] listeners = searchInConstantPoolsFileMaskComboBox.getActionListeners();
                for (ActionListener l : listeners) {
                    searchInConstantPoolsFileMaskComboBox.removeActionListener(l);
                }

                searchInConstantPoolsFileMaskComboBox.removeAllItems();
                String[] fileMaskPresets = {"*", "*.class", "*.xml", "*.properties", "*.json", "*.yml;*.yaml", "*.txt"};
                Set<String> set = new LinkedHashSet<>();
                if (history != null) {
                    set.addAll(history);
                }
                set.addAll(Arrays.asList(fileMaskPresets));
                for (String item : set) {
                    searchInConstantPoolsFileMaskComboBox.addItem(item);
                }

                searchInConstantPoolsFileMaskComboBox.setSelectedItem(currentMask != null ? currentMask : "*");

                // Restore action listeners
                for (ActionListener l : listeners) {
                    searchInConstantPoolsFileMaskComboBox.addActionListener(l);
                }
            } finally {
                isPopulatingHistory = false;
            }
